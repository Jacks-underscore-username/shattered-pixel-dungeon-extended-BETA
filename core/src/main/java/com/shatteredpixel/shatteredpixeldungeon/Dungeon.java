 ...
 */
 ...
 public class Dungeon {
	 ...
	 public static Level newLevel() {
		 ...
		 if (isChallenged(Challenges.OVERPOPULATION)) {
			 for (int x = 0; x < level.width(); x++) {
				 for (int y = 0; y < level.height(); y++) {
					 //enemies cannot be within an 8-tile FOV of the entrance
					 // or a 6-tile open space distance from the entrance
					 boolean[] entranceFOV = new boolean[level.length()]; 
					 Point c = level.cellToPoint(level.entrance());
					 ShadowCaster.castShadow(c.x, c.y,level. width(), entranceFOV, level.losBlocking, 6);
					 PathFinder.buildDistanceMap(level.entrance(), BArray.not(level.solid, null), 8);
 
					 Mob mob = level.createMob();
 
					 mob.pos = level.pointToCell(new Point(x, y));
					 boolean canSpawn =  !((
							 ...
							 || level.plants.get(mob.pos) != null
							 || (!level.openSpace[mob.pos] && mob.properties().contains(Char.Property.LARGE))));
